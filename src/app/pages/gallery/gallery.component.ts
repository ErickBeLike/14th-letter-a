...
export class GalleryComponent implements OnInit {
  ...
  memories: Memory[] = [
    ...
    },
    {
      ...
      description: 'Y la guinda del pastel, el encuentro en sí. Mira que ya lo he contado varias veces pero siendo sincero, no me cansaría de hacerlo, todo empezó como pensamientos e ideas vagas en donde me decía "jolines tío, estaría flipante conocerla en persona", quizá sin el acento castellano, jeje, pero empezó ese pensamiento como una posibilidad un tanto casual. Pero literalmente a "poco" de haber tenido esa epifanía, se dio la oportunidad en forma de píldora roja o azul que me contaste, en donde al final por situaciones de la vida (y ayuda del guión) se dio paso al plan de visita-vacaciones. No voy a negar que los días posteriores a la confirmación fueron un poquito caóticos en mi mente (aunque por lo que se habló, en la mente de los dos), al menos en el sentido de que "santos cielos, van a venir, ¿qué haré?, ¿cómo me comportaré?, ¿qué tal si no nos llevamos bien en persona?" pero era súper raro porque lo dicho, iba a conocer en persona a alguien que conocía digitalmente de hace mucho pero que a su vez no conoces en NADA, iba a ser la etapa final de darle cuerpo, forma y cara a esa skin de Roblox que tantos años miré, creo fue mútua esa sensación. Llegado el día, pese a los nervios iniciales todo fluyó de manera muy natural, con detallitos de por medio pero nada grave, lo seguiré diciendo, que el local de "esquites" estuviese abierto fue una ayuda divina. Los días posteriores fueron literalmente toda una mini aventura a lo Roblox, estuvimos en un castillo medieval, volvimos a la prehistoria, vimos dragones, estuvimos en la playita, tuvimos un imperio, perdimos un imperio, solo nos faltó un tycoon, pero vaya, creo que puedo dar a entender que esos poquitos pero hermosos 3 días, fueron literalmente TODA una experiencia. Acerca del desenlace, he de decir que el destino me pasó el balón de pechito para que la despedida se sintiese bonito y heróico, desde unas florecitas así casuales para ti y tu mami hasta una salvada histórica (tenía que hacerme merecedor de mi armadura) así como lo dije en su entonces, final con broche de oro. En esos días hubo varias foticos de las cuales no sabes cómo me alegran el día y el corazón cada que las veo, son como nuestras screenshots de Roblox pero en 4K, para mi, el recuerdo de algo muy lindo a decir verdad, de las cuales no está de más decir que la que se ganó el podio en mi mente y corazón fue la del balcón, así como los dibujos, no quiero dar preferencia pero es que esa me hace unos ojitos que fua chaval. En fin, gracias tanto a ti como a tu mamá por haber dado la oportunidad de que se diese esa experiencia, tocará ver cuándo y cómo se hace "El encuentro, VOL. 2". :D.',
    },
  ];

  ngOnInit() {
    // Aseguramos que suene la romántica.
    // Si ya está sonando (porque vienes del sobre), no pasa nada.
    // Si vienes del chat (Minecraft), hará la transición suave.
  ...
  navigateToBot() {
    // 1. PRIMERO: Disparar el glitch de audio INMEDIATAMENTE (antes que cualquier otra cosa)
    this.audioService.stopWithGlitch();

    // 2. Activamos el modo caos visual
    this.isGlitchingOut.set(true);

    // 3. Esperamos 800ms (lo que dura la animación) antes de cambiar de ruta
    setTimeout(() => {
